...
class RowHeaderFieldsArea(tableDataProperty:SimpleObjectProperty[TableData], dragAndDrop:DragAndDrop) extends FlowPane with DropTargetContainer with DropTarget with DraggableParent {
  ...
  def fieldsDropped(draggableFieldsInfo:DraggableFieldsInfo, tableData:TableData) = {
    ...
    val newRowFields = currentTableState.tableLayout.rowHeaderFields ++ draggableFieldsInfo.fields
  ...
  def removeFields(draggableFieldsInfo:DraggableFieldsInfo, tableData:TableData) = {
    val updatedFields = tableData.tableState.tableLayout.rowHeaderFields.filterNot(draggableFieldsInfo.fields.contains)
